...
scraper.shoppingnt = {
    ...
    getDOrderList: async function() {
        try {
            ...
            scraper.shoppingnt.excelDownload();
        }
        catch(error) {
            await cAPI.error(error.message);
        }
    ...
    getReturnList2: async function() {
        try {
            ...
            select[0].click();
            await scraper.delay(1500);
            var selectChk = await scraper.getElement('li:contains("고객직접발송")', 10000, 'Y');
            selectChk.click();                                                                     
